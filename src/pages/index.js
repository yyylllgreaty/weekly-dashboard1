...
function sw(w){return(w||"").replace(/ 20\d\d/,"").replace("W0","W")+" '26";}
...
function ChartTip({active,payload,label,dateRanges,weekLabels}){
  if(!active||!payload||!payload.length) return null;
  var idx=weekLabels?weekLabels.indexOf(label):-1;
  return(
    <div style={{background:"#fff",border:"1px solid "+CC.bdr,borderRadius:8,padding:"10px 14px",boxShadow:"0 4px 16px rgba(0,0,0,0.08)",fontFamily:FF}}>
      <div style={{fontWeight:700,fontSize:13,color:CC.tx}}>{label}</div>
      {idx>=0&&dateRanges&&<div style={{fontSize:11,color:CC.txM,marginBottom:6}}>{dateRanges[idx]}</div>}
...
function LeadsChart({data,color,soldLabel,dateRanges,weekLabels}){
  return(<ResponsiveContainer width="100%" height={250}><ComposedChart data={data} barGap={2}>
    ...
    <Tooltip content={<ChartTip dateRanges={dateRanges} weekLabels={weekLabels}/>}/>
...
function TLChart({data,dateRanges,weekLabels}){
  return(<ResponsiveContainer width="100%" height={250}><ComposedChart data={data} barGap={2}>
    <CartesianGrid strokeDasharray="3 3" stroke={CC.bdrL} vertical={false}/>
    <XAxis dataKey="wk" tick={{fill:CC.txM,fontSize:10,fontFamily:FF}} axisLine={{stroke:CC.bdr}} tickLine={false}/>
    <YAxis yAxisId="l" tick={{fill:CC.txM,fontSize:10}} axisLine={false} tickLine={false}/>
...
function PWChart({data,dateRanges,weekLabels}){
...
function StateTable({states,w,weeks}){
  if(!states||Object.keys(states).length===0) return null;
  var wkL=weeks.map(sw);
  return(
    <div style={{overflowX:"auto",borderRadius:10,border:"1px solid "+CC.bdr,background:CC.card}}>
      <table style={{width:"100%",borderCollapse:"collapse",fontFamily:FF,fontSize:12.5}}>
        <thead><tr><th style={{...thS,textAlign:"left"}}>State</th><th style={thS}>Metric</th>
          {wkL.map(function(wl,ci){return(<th key={ci} style={{...thS,minWidth:48,background:ci===w?"#EFF6FF":"transparent",color:ci===w?CC.blu:CC.txL}}>{wl}</th>);})}
        </tr></thead>
...
function BizPage({name,d,w,color,soldLabel,showPW,weeks,dateRanges,states}){
  var sl=soldLabel||"Leads Sold";
  var wkL=weeks.map(sw);
  var cd=weeks.map(function(_,i){return{wk:wkL[i],gen:d.gen[i],sold:d.sold[i],sp:d.sp[i],tlS:d.tlS[i],tlC:d.tlC[i],tlR:d.tlR[i],pwS:d.pwS?d.pwS[i]:null,pwC:d.pwC?d.pwC[i]:null,pwR:d.pwR?d.pwR[i]:null};});
  return(
    <div>
      <h1 style={{fontSize:24,fontWeight:700,color:CC.tx,margin:"0 0 4px",fontFamily:FF}}>{name}</h1>
      <p style={{fontSize:13,color:CC.txM,margin:"0 0 20px",fontFamily:FF}}>Weekly performance · {weeks[w]} · {dateRanges[w]}</p>
      <div style={{display:"flex",gap:12,marginBottom:22,flexWrap:"wrap"}}>
        <KPI label={"Leads Generated ("+weeks[w]+")"} val={d.gen[w]} dv={vpct(d.gen[w],a5(d.gen,w))}/>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:22}}>
        <div style={{background:CC.card,borderRadius:10,border:"1px solid "+CC.bdr,padding:"18px 18px 10px"}}>
          ...
          <LeadsChart data={cd} color={color} soldLabel={sl} dateRanges={dateRanges} weekLabels={wkL}/>
        </div>
        <div style={{background:CC.card,borderRadius:10,border:"1px solid "+CC.bdr,padding:"18px 18px 10px"}}>
          ...
          <TLChart data={cd} dateRanges={dateRanges} weekLabels={wkL}/>
        </div>
      </div>
      {showPW&&(<div style={{background:CC.card,borderRadius:10,border:"1px solid "+CC.bdr,padding:"18px 18px 10px",marginBottom:22}}>
        ...
        <PWChart data={cd} dateRanges={dateRanges} weekLabels={wkL}/>
      </div>)}
      <div style={{overflowX:"auto",borderRadius:10,border:"1px solid "+CC.bdr,background:CC.card,marginBottom:22}}>
        <table style={{width:"100%",borderCollapse:"collapse",fontFamily:FF,fontSize:12.5}}>
          ...
          <tbody>{[...weeks].reverse().map(function(wk,ri){var i=weeks.length-1-ri;var sel=i===w;return(
            <tr key={i} style={{borderBottom:"1px solid "+CC.bdrL,background:sel?"#EFF6FF":"transparent"}}>
              <td style={{padding:"7px 12px",fontWeight:600,color:CC.tx,fontSize:12.5}}>{wkL[i]}</td>
      ...
      {states&&Object.keys(states).length>0&&(<><div style={{fontSize:11,fontWeight:700,color:CC.txL,letterSpacing:0.8,textTransform:"uppercase",marginBottom:8,fontFamily:FF}}>By State</div><StateTable states={states} w={w} weeks={weeks}/></>)}
    </div>
  );
}

function MACombined({w,t1,t23,weeks,dateRanges,states}){
  var wkL=weeks.map(sw);
  var cd1=weeks.map(function(_,i){return{wk:wkL[i],gen:t1.gen[i],sold:t1.sold[i],sp:t1.sp[i],tlS:t1.tlS[i],tlC:t1.tlC[i],tlR:t1.tlR[i]};});
  var cd23=weeks.map(function(_,i){return{wk:wkL[i],gen:t23.gen[i],sold:t23.sold[i],sp:t23.sp[i]};});
  return(
    <div>
      <h1 style={{fontSize:24,fontWeight:700,color:CC.tx,margin:"0 0 4px",fontFamily:FF}}>MyAccident</h1>
      <p style={{fontSize:13,color:CC.txM,margin:"0 0 22px",fontFamily:FF}}>Tier 1 & Tier 2/3 · {weeks[w]} · {dateRanges[w]}</p>
      ...
      <div style={{display:"flex",gap:12,marginBottom:20,flexWrap:"wrap"}}>
        <KPI label={"Leads Generated ("+weeks[w]+")"} val={t1.gen[w]} dv={vpct(t1.gen[w],a5(t1.gen,w))}/>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:22}}>
        <div style={{background:CC.card,borderRadius:10,border:"1px solid "+CC.bdr,padding:"18px 18px 10px"}}><div style={{fontSize:11,fontWeight:700,color:CC.txL,letterSpacing:0.8,textTransform:"uppercase",marginBottom:10,fontFamily:FF}}>Leads Volume & Sell-Through</div><LeadsChart data={cd1} color={CC.org} dateRanges={dateRanges} weekLabels={wkL}/></div>
        <div style={{background:CC.card,borderRadius:10,border:"1px solid "+CC.bdr,padding:"18px 18px 10px"}}><div style={{fontSize:11,fontWeight:700,color:CC.txL,letterSpacing:0.8,textTransform:"uppercase",marginBottom:10,fontFamily:FF}}>TL Pipeline & Conversion</div><TLChart data={cd1} dateRanges={dateRanges} weekLabels={wkL}/></div>
      </div>
      {states&&Object.keys(states).length>0&&(<><div style={{fontSize:11,fontWeight:700,color:CC.txL,letterSpacing:0.8,textTransform:"uppercase",marginBottom:8,fontFamily:FF}}>Tier 1 — By State</div><StateTable states={states} w={w} weeks={weeks}/></>)}
      ...
      <div style={{display:"flex",gap:12,marginBottom:20,flexWrap:"wrap"}}>
        <KPI label={"Leads Generated ("+weeks[w]+")"} val={t23.gen[w]} dv={vpct(t23.gen[w],a5(t23.gen,w))}/>
      ...
      <div style={{background:CC.card,borderRadius:10,border:"1px solid "+CC.bdr,padding:"18px 18px 10px",marginBottom:22}}><div style={{fontSize:11,fontWeight:700,color:CC.txL,letterSpacing:0.8,textTransform:"uppercase",marginBottom:10,fontFamily:FF}}>Leads Volume & Sell-Through</div><LeadsChart data={cd23} color={CC.pur} dateRanges={dateRanges} weekLabels={wkL}/></div>
    </div>
  );
}

function Overview({w,data}){
  var weeks=data.weeks,dateRanges=data.dateRanges,lgm=data.lgm,spz=data.spz,t1=data.maTier1,t23=data.maTier23;
  var wkL=weeks.map(sw);
  var cd=weeks.map(function(_,i){
    ...
    return{wk:wkL[i],tg:tg,ts:ts,sp:tg?(ts/tg)*100:0,tSent:tSent,tSig:tSig,tConv:tSent>0?(tSig/tSent)*100:0,lgmS:lgm.sold[i],spzS:spz.sold[i],ma1S:t1.sold[i],ma23S:t23.sold[i]};
  });
  ...
  return(
    <div>
      ...
      <p style={{fontSize:13,color:CC.txM,margin:"0 0 20px",fontFamily:FF}}>All businesses combined · {weeks[w]} · {dateRanges[w]}</p>
      <div style={{display:"flex",gap:12,marginBottom:22,flexWrap:"wrap"}}>
        <KPI label={"Total Generated ("+weeks[w]+")"} val={c.tg} dv={vpct(c.tg,a5(tga,w))}/>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:22}}>
        <div style={{background:CC.card,borderRadius:10,border:"1px solid "+CC.bdr,padding:"18px 18px 10px"}}>
          ...
          <ResponsiveContainer width="100%" height={250}><ComposedChart data={cd} barGap={1}>
            <CartesianGrid strokeDasharray="3 3" stroke={CC.bdrL} vertical={false}/><XAxis dataKey="wk" tick={{fill:CC.txM,fontSize:10}} axisLine={{stroke:CC.bdr}} tickLine={false}/><YAxis tick={{fill:CC.txM,fontSize:10}} axisLine={false} tickLine={false}/><Tooltip content={<ChartTip dateRanges={dateRanges} weekLabels={wkL}/>}/><Legend wrapperStyle={{fontSize:11,fontFamily:FF}}/>
        ...
        <div style={{background:CC.card,borderRadius:10,border:"1px solid "+CC.bdr,padding:"18px 18px 10px"}}>
          ...
          <ResponsiveContainer width="100%" height={250}><ComposedChart data={cd} barGap={2}>
            ...
            <Tooltip content={<ChartTip dateRanges={dateRanges} weekLabels={wkL}/>}/><Legend wrapperStyle={{fontSize:11,fontFamily:FF}}/>
      ...
      <div style={{overflowX:"auto",borderRadius:10,border:"1px solid "+CC.bdr,background:CC.card}}>
        <table style={{width:"100%",borderCollapse:"collapse",fontFamily:FF,fontSize:12.5}}>
          ...
          <tbody>{[...weeks].reverse().map(function(wk,ri){var i=weeks.length-1-ri;var x=cd[i];var sel=i===w;return(
            <tr key={i} style={{borderBottom:"1px solid "+CC.bdrL,background:sel?"#EFF6FF":"transparent"}}>
              <td style={{padding:"7px 12px",fontWeight:600,color:CC.tx,fontSize:12.5}}>{wkL[i]}</td>
...
export default function Home(){
  ...
  if(loading) return(<div style={{display:"flex",alignItems:"center",justifyContent:"center",minHeight:"100vh",background:CC.bg,fontFamily:FF}}><div style={{textAlign:"center"}}><div style={{width:40,height:40,borderRadius:10,background:"linear-gradient(135deg,#2563EB,#7C3AED)",margin:"0 auto 16px",display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,fontWeight:800,color:"#fff"}}>WR</div><div style={{fontSize:16,fontWeight:600,color:CC.tx}}>Loading dashboard...</div><div style={{fontSize:13,color:CC.txM,marginTop:4}}>Fetching data from Google Sheets</div></div></div>);
  ...
  var weeks=data.weeks,dateRanges=data.dateRanges,wkL=weeks.map(sw);
  var lastUp=data.lastUpdated?new Date(data.lastUpdated).toLocaleDateString("en-US",{month:"short",day:"numeric",year:"numeric"}):"—";
  ...
  return(
    ...
    <div style={{display:"flex",minHeight:"100vh",background:CC.bg,fontFamily:FF}}>
      {!sc&&(<div style={{width:210,minWidth:210,background:"#fff",borderRight:"1px solid "+CC.bdr,padding:"18px 12px",display:"flex",flexDirection:"column"}}>
        ...
        <div style={{flex:1}}/>
        <div style={{padding:"10px 8px",borderTop:"1px solid "+CC.bdr,marginTop:12}}><div style={{fontSize:11,color:CC.txM}}>Last updated</div><div style={{fontSize:12,color:CC.tx2,fontWeight:600}}>{lastUp}</div></div>
      </div>)}
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"12px 28px",borderBottom:"1px solid "+CC.bdr,background:"#fff",position:"sticky",top:0,zIndex:50}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            ...
            <select value={w} onChange={function(e){setW(Number(e.target.value));}} style={{background:"#fff",color:CC.tx,border:"1px solid "+CC.bdr,borderRadius:6,padding:"5px 10px",fontSize:12.5,fontWeight:600,fontFamily:FF,cursor:"pointer",outline:"none"}}>
              {weeks.map(function(wk,i){return(<option key={i} value={i}>{wkL[i]} — {dateRanges[i]}</option>);})}
            </select>
          </div>
        </div>
        <div style={{padding:"24px 28px 44px",maxWidth:1200}}>
          {pg==="overview"&&<Overview w={w} data={data}/>}
          {pg==="lgm"&&<BizPage name="LGM — Legal Growth Marketing" d={data.lgm} w={w} color={CC.org} showPW={true} weeks={weeks} dateRanges={dateRanges} states={data.lgmStates}/>}
          {pg==="spz"&&<BizPage name="SpringZip" d={data.spz} w={w} color={CC.org} soldLabel="Sold (excl. LGM)" showPW={true} weeks={weeks} dateRanges={dateRanges} states={data.spzStates}/>}
          {pg==="ma"&&<MACombined w={w} t1={data.maTier1} t23={data.maTier23} weeks={weeks} dateRanges={dateRanges} states={data.maStates}/>}
        </div>
      </div>
    </div></>
  );
